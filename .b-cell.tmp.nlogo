...
to naive-b-cell-function
  set cd21-level 200 - (il6 * 50 + il10 * 0)     ; Calculates CD21 exppression based off of il6 and il10 levels (scaled) in current patch
...
to gc-b-cell-function
  if in-blood = false [
    ...
    ifelse distance patch 0 0 > 15 [
      ...
      if time-alive mod 0 = 0 [  ; For now, hard-coded to represent rate of proliferatoin. Should be influenced by cytokines from state diagram
        ifelse proPC > proMem [
          ifelse ticks < 2800 [
...
to mem-b-cell-function
  ...
  set cd21-level 200 - (il6 * 50 + il10 * 0)
